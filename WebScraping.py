...
def conectionDB(book):
    if book != []:
        ...
        objQuery.execute(
            'INSERT INTO public.auditoria( "codigo", "fecha", "pagina_web", "numero_registros", "estado", "errores") VALUES(%s, %s, %s, %s, %s, %s);',
            (int(book[7]), time.strftime("%d/%m/%y"),"www.casadellibro.com", count,"finalizado", "no"))
